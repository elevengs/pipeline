...
def midline(
    ...
    midline_al = [
        0,
        *np.cumsum(
            np.sqrt(np.sum(np.square(np.diff(midline_coordinates, axis=0)), axis=1))
        ),
    ]
    ...
    mid_spline = make_interp_spline(midline_al, midline_coordinates)
    ...
    upper_als = np.linspace(
        midline_length, midline_length + extension_length, num=n_extension
    )[1:]
    ...
    extended_al = [*lower_als, *midline_al, *upper_als]

    # Cell center should by roughly zero on the midline
    extended_al = [x - (midline_length / 2) for x in extended_al]

    extended_points = np.r_[lower_points, midline_coordinates, upper_points]
