...
def extend_spine(spine, skin):
    ...
    mag = np.sqrt((v**2).sum())
    probe = LineString(np.array([[x0,y0], (diag/(mag + 0.0001))*v + np.array([x0,y0])]))
...
def get_skel_coords(skel):
    ...
    while ~(curr_pt[0] == end_pt[0] and curr_pt[1] == end_pt[1]):
        ...
        if not anything_changed:
            ...
            # Not sure what causes this yet (TODO) but
            # we definitely need to get out of it
            raise Exception("infinite loop detected in get_skel_coords")
...
def get_spine(mask):
    ...
    # binary_dilation is deprecated
    # mask_smooth = binary_dilation(mask, footprint=disk(2))
    mask_smooth = dilation(mask, footprint=disk(2))
